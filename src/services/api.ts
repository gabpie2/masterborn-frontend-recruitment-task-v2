...
export async function calculatePrice(
  ...
  return {
    ...
    // BUG: This timestamp is used to track which response is "newest"
    // but we're using requestId which can arrive out of order
    timestamp: requestId,
  };
}
...
export function encodeConfigurationToUrl(config: Configuration): string {
  // This is the buggy version - doesn't properly encode special characters
  ...
  // BUG: Using btoa without proper UTF-8 handling
  // This will break with special characters like quotes, Unicode, etc.
  return btoa(data);
}
