...
// Discount tiers
// BUG: Off-by-one in the threshold check (uses > instead of >=)
...
export function calculatePriceBreakdown(
  ...
  // This can cause issues like 19.99 + 5.01 = 25.000000000000004
  ...
  // BUG: Uses > instead of >= for threshold check (off-by-one)
  let discountRate = 0;
...
export function getAppliedDiscountPercentage(quantity: number): number {
  for (const tier of QUANTITY_DISCOUNT_TIERS) {
    // Same bug as above - should be >= not >
...
export function getNextDiscountTier(quantity: number): { needed: number; discount: number } | null {
  ...
  for (const tier of sortedTiers) {
    // BUG: This uses >= which is inconsistent with the actual discount check
    if (quantity < tier.minQuantity) {
      return {
